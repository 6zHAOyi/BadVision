...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.model_caption_loader \
        ...
        --question-file ./playground/data/eval/flickr/flickr_caption_2k_llava.jsonl \
        --image-folder ./eval_data/flickr30k \
        --answers-file ./playground/data/eval/flickr/${CHUNKS}_${IDX}.jsonl \
...
output_file=./playground/data/eval/flickr/merge.jsonl
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat ./playground/data/eval/flickr/${CHUNKS}_${IDX}.jsonl >> "$output_file"
done
