...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.model_caption_loader \
        ...
        --answers-file ./playground/data/eval/gqa/${CHUNKS}_${IDX}_cap.jsonl \
...
output_file=./playground/data/eval/gqa/caption.jsonl
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat ./playground/data/eval/gqa/${CHUNKS}_${IDX}_cap.jsonl >> "$output_file"
done